...
export default function AboutTabs() {
  ...
  return (
    <section className="pt-6 md:pt-8 lg:pt-10" style={{ backgroundColor: '#f5f5f5' }}>
      <div className="grid md:grid-cols-2 gap-0 min-h-[400px] sm:min-h-[480px] md:min-h-[520px]">
        {/* Left Side - Tabs with Background Image */}
        <div
          className="relative bg-cover bg-center transition-all duration-700 ease-in-out min-h-[400px] sm:min-h-[480px] md:min-h-0"
          style={{
        ...
        >
          ...
          {/* Tabs */}
          <div className="relative z-10 flex flex-col justify-center h-full py-6 sm:py-8 space-y-1 sm:space-y-2" style={{ marginLeft: 'auto', width: 'fit-content', maxWidth: '85%', paddingRight: '1rem' }}>
            {tabData.map((tab) => (
              <button
                key={tab.id}
                ...
                style={{
                  backgroundColor: activeTab === tab.id ? 'rgba(249, 178, 51, 0.3)' : 'transparent',
                  minHeight: '60px'
                }}
              >
                {/* Check Icon Circle */}
                <div
                  className="flex-shrink-0 w-10 h-10 sm:w-10 sm:h-10 md:w-12 md:h-12 rounded-full border-2 flex items-center justify-center transition-all duration-300"
                  style={{
                    borderColor: activeTab === tab.id ? '#f5f5f5' : '#f9b233',
                    backgroundColor: 'transparent'
                  }}
                >
                  <Check
                    className="w-5 h-5 sm:w-5 sm:h-5 md:w-6 md:h-6 transition-colors duration-300"
                    style={{ color: activeTab === tab.id ? '#f5f5f5' : '#f9b233' }}
                    strokeWidth={2.5}
                  />
                </div>
                ...
                <div className="flex flex-col gap-0.5">
                  <span
                    className="text-sm sm:text-base md:text-lg font-medium uppercase tracking-wide transition-colors duration-300 leading-tight"
                    style={{
                      fontFamily: '"Black Ops One", sans-serif',
                      color: activeTab === tab.id ? '#f5f5f5' : '#f9b233'
                    }}
                  >
                    {tab.title}
                  </span>
        ...
        {/* Right Side - Content */}
        <div
          className="flex flex-col justify-center p-6 sm:p-8 md:p-12 lg:p-16 min-h-[200px]"
